...
class WordLineDataset(Dataset):
    ...
    def __finalize__(self):
        ...
        save_path = './UNHD_dataset_PIL_style'
        if os.path.exists(save_path) is False:
            os.makedirs(save_path, exist_ok=True)

        save_file = '{}/{}_{}_{}.pt'.format(save_path, self.subset, 
                                           self.segmentation_level, self.setname)
        print('save_file', save_file)
    ...
    def __getitem__(self, index):
        ...
        if len(positive_samples) >= 5:
            ...
            if k > 0:
                ...
                # Fallback: use current image 5 times
                style_images = [img] * 5
        ...
        # Process main image - resize to fixed size
...
class UNHDDataset_style(WordLineDataset):
    def __init__(self, basefolder, subset, segmentation_level, fixed_size, transforms):
        super().__init__(basefolder, subset, segmentation_level, fixed_size, transforms)
        self.setname = 'UNHD'
    ...
    def main_loader(self, subset, segmentation_level):
        def gather_unhd_info(self, set_type='train', level='line'):
            if set_type == 'train':
                valid_set = np.loadtxt(self.trainset_file, dtype=str)
            elif set_type == 'val':
                valid_set = np.loadtxt(self.valset_file, dtype=str)
                ...
                raise ValueError(f"Unknown subset: {set_type}")
        ...
        for i, (img_path, transcr, writer_id) in enumerate(info):
            if i % 1000 == 0:
                print('imgs: [{}/{} ({:.0f}%)]'.format(i, len(info), 100. * i / len(info)))
            
            try:
                full_path = img_path if img_path.endswith('.png') else img_path + '.png'
                img = Image.open(full_path).convert('RGB')
                
                # Don't resize here - will be done in __getitem__
...
class Mixed_Encoder(nn.Module):
    def __init__(self, model_name='resnet50', num_classes=421, pretrained=True, trainable=True):
        super().__init__()
...
def train_triplet(model, train_loader, val_loader, criterion, optimizer, scheduler, device, args):
    ...
    # Create save directory at the start
...
def main():
    parser = argparse.ArgumentParser(description="Train Style Encoder for DiffusionPen")
    parser.add_argument('--data_path', type=str, default='./unhd_data/UNHD-Complete-Data',
                       help='Path to UNHD data directory')
    parser.add_argument('--model_type', type=str, default='mobilenetv2_100',
                       choices=['resnet18', 'resnet50', 'mobilenetv2_100', 'efficientnet_b0'],
    ...
    parser.add_argument('--save_path', type=str, default='./style_models/unhd_triplet.pth',
                       help='Path to save the trained style encoder')
    ...
    print(f"\n{'='*60}")
    print(f"DiffusionPen Style Encoder Training")
    print(f"{'='*60}")
    ...
    train_triplet(model, train_loader, val_loader, criterion_triplet, optimizer, scheduler, device, args)
